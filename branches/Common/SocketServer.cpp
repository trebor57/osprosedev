...
bool CClientSocket::ISCThread()
{
	...
	//Log( MSG_INFO, "ISC Communication Active" );
	...
	do {
		...
		if( recvd == 0){
			//Log( MSG_ERROR, "Server Disconected!" );
			return false;
		}

		if( recvd == SOCKET_ERROR ) {
			//Log( MSG_INFO, "Server Disconected!" );
			return false;
		}
		...
		//CryptISCPak( &buffer[0] );
		//GS->ReceivedISCPacket( (CPacket*)&buffer[0] );
		GS->OnReceivePacket( this, (CPacket*)&buffer[0] );
		go = false;
		//Log( MSG_INFO, "ISC Communication finished" );
...
void CServerSocket::HandleClients( fd_set* fds )
{
    for(UINT i=0;i<ClientList.size( );i++)
	{
		...
		{//Log( MSG_INFO,"port %s",inet_ntoa( client->clientinfo.sin_addr ));
		//Log( MSG_INFO,"Info %d", ntohs(client->clientinfo.sin_port));
		//Log( MSG_INFO,"Info %d", ntohs(ain.sin_port));
