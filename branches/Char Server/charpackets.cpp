...
{    
	...
	ain.sin_family		= AF_INET;
   	ain.sin_addr.s_addr	= inet_addr(newchannel->ip);//thisclient->clientinfo.sin_addr.s_addr;
...
/*    
    ...
    switch(action)
    {
        ...
        {
            char username[65];
            memcpy( username, &P->Buffer[1], P->Size );               
            CCharClient* otherclient = GetClientByUserName( username );         
            if(otherclient==NULL)
            {
                Log( MSG_WARNING, "User '%u' is not online", username );
                return true;
            }
            
                if(!DB->QExecute( "UPDATE accounts SET online=0 WHERE username='%s'", otherclient->username ))
        return false;
	otherclient->isLoggedIn = false;
            otherclient->isActive = false;
