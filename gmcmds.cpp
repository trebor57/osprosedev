...
bool CWorldServer::pakGMCommand( CPlayer* thisclient, CPacket* P )
{
   ...
   else if (strcmp(command, "mute")==0) //==== Mute a player ==== [by Paul_T]
   {
        ...
        for(int i=0;i<10;i++)
            {
                if (thisclient->quest.quests[i].QuestID != 0) continue;
                    ...
                    thisclient->quest.quests[i].Time = clock_t();
    ...
    else if(strcmp(command, "listquest")==0)
    {
        ...
        if(Config.Command_IQuest > thisclient->Session->accesslevel)
	         return true;
        Log( MSG_GMACTION, "list quests" );
        for(int i=0;i<10;i++)
            {
                Log( MSG_GMACTION, "quest %i %i %i %i %i %i" , thisclient->quest.quests[i].QuestID,thisclient->quest.quests[i].Items[0],thisclient->quest.quests[i].Items[1],thisclient->quest.quests[i].Items[2],thisclient->quest.quests[i].Items[3],thisclient->quest.quests[i].Items[4]);
            }
    }
