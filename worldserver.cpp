...
//    }
    ...
    if(!DB->QExecute("INSERT INTO channels (id,name,host,port,lanip,lansubmask,connected,maxconnections,owner) VALUES (%i,'%s','%s',%u,'%s','%s',0,%i,%i)",
            Config.ServerID, Config.ServerName, Config.WorldIP, Config.WorldPort, Config.LanIP, Config.LanSubnet, Config.MaxConnections, Config.ParentID))
    {
        Log(MSG_WARNING, "Error accessing to database, the other server will not connect to WorldServer" );
    }
  ...
  /*
    ...
    MYSQL_RES *result = DB->QStore( "SELECT host,port,lanip FROM channels WHERE id=%u", Config.ParentID );
...
/*        CPacket *P=&pak;
	    ...
	    send( csock, (char*)Buffer, Size, 0 );
        //EncryptBuffer(CryptTable,pak);
    	//cryptPacket( (char*)&pak, NULL );
    	//send( csock, (char*)&pak, pak.Size, 0 );
    }
    */
...
bool CWorldServer::pakTeleport( CPlayer* thisclient, unsigned short map, float x, float y )
{
	...
	GServer->MapList.Index[thisclient->Position->Map]->RemovePlayer( thisclient, false );

	ClearClientID( thisclient->clientid );
	thisclient->Position->Map = map;
	thisclient->Position->destiny.x = x;
	thisclient->Position->destiny.y = y;
	thisclient->Position->current = thisclient->Position->destiny;
	thisclient->Status->Stance= 0;

	pakout.StartPacket( 0x7a8 );
	pakout.AddWord( thisclient->clientid );
	pakout.AddWord( thisclient->Position->Map );
	pakout.AddFloat( thisclient->Position->current.x );
	pakout.AddFloat( thisclient->Position->current.y );
	pakout.AddWord( 1 );
	thisclient->client->SendPacket( &pakout );*/
	 return true;
}

bool CWorldServer::pakNPCAction( CPlayer* thisclient, CPacket* pak )
{
word action = pak->GetByte( 0 );
if( thisclient->questdebug ) Log( MSG_DEBUG, "action %02x",action);
dword hash = pak->GetDWord( 2 );
    
if( thisclient->questdebug ) Log( MSG_INFO, "event trigger %d",hash);
...
unsigned char hack1[6] = {0x03, 0x00, 0x79, 0x8e, 0xec, 0x25};
unsigned char hack2[6] = {0x03, 0x00, 0x99, 0x00, 0x00, 0x00};
unsigned char hack3[6] = {0x03, 0x00, 0x8d, 0x19, 0xba, 0x46};
unsigned char hack4[6] = {0x03, 0x00, 0x7b, 0xfa, 0x33, 0x3d};
unsigned char hack5[6] = {0x03, 0x00, 0x3e, 0xb4, 0xad, 0x5f};
unsigned char hack6[6] = {0x03, 0x00, 0x9f, 0x1d, 0x5f, 0x1c};
unsigned char hack7[6] = {0x03, 0x00, 0xc6, 0xa1, 0x5c, 0x1c};
/*
          if( memcmp( &pak->Buffer, &hack1, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack2, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack3, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack4, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack5, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack6, 6 ) == 0 )
                   isHacking = true;
          else if( memcmp( &pak->Buffer, &hack7, 6 ) == 0 )
                   isHacking = true;
*/
          if( isHacking == true )
          {
              Log( MSG_HACK, "Char '%s' is quest packet hacking", thisclient->CharInfo->charname );
              isHacking == false;
              return true;
          }
      ...
      {
          //return 
          nextquestAction(thisclient, hash);
       }
       else if( action == 2 )
      {
            for( int i=0; i<10; i++ )
            if( thisclient->quest.quests[i].QuestID == hash )
                memset(&thisclient->quest.quests[i], 0, sizeof(SQuest));
      }
    CPacket pakout(0x730);
    if(action==2)pakout.AddWord(pak->GetWord(0)+1);
    else pakout.AddWord((action==3)?5:3);
    pakout.AddDWord(hash);
    thisclient->client->SendPacket(&pakout);
	
	return true;
}
bool CWorldServer::nextquestAction( CPlayer* thisclient, DWORD hash )
{
    CQuestTrigger* trigger;
    CQuestTrigger* nexttrigger;
    for(unsigned j=0; j<TriggerList.size(); j++)
    {
      trigger=TriggerList.at(j);
      if (trigger->TriggerHash==hash)
      {
          nexttrigger=TriggerList.at(j+1);
          break;
      }
    }
    #ifdef debugging
    Log(MSG_INFO, "trigger name %s",trigger->TriggerName);
    #endif
    for(dword i = 0; i < trigger->ConditionCount; i++)
    {
        ...
        {
            if(!(trigger->CheckNext))return true;
            return nextquestAction(thisclient, nexttrigger->TriggerHash);
        }
    }
    for(dword i = 0; i < trigger->ActionCount; i++)
    {
        ...
        if(command == 9)
        {
            char nexttriggername[trigger->Actions[i]->size-2];
            for (unsigned int j=0;j<(trigger->Actions[i]->size-2);j++)
                nexttriggername[j]=trigger->Actions[i]->data[j+2];
            nexttriggername[trigger->Actions[i]->size-2]=0;
            DWORD nexthash = MakeStrHash(nexttriggername);
            nextquestAction(thisclient, nexthash);
            return true;
        }
...
{//Log( MSG_INFO, "(SID:%i) Received packet. Command:%04x Size:%04x", thisclient->sock, P->Command, P->Size );
	switch( P->Command )
	{
    	...
    	case 0x0730: return pakNPCAction        ( (CPlayer*)thisclient->player, P );
