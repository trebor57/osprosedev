...
//    }
    ...
    if(!DB->QExecute("INSERT INTO channels (id,name,host,port,lanip,lansubmask,connected,maxconnections,owner) VALUES (%i,'%s','%s',%i,'%s','%s',0,%i,%i)", 
            Config.ServerID, Config.ServerName, Config.CharIP, Config.CharPort, Config.LanIP, Config.LanSubnet, Config.MaxConnections, Config.ParentID))    
    {
        Log(MSG_WARNING, "Error accessing to database, the other server will not connect to CharServer" );
    }
/*
