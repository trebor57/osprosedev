...
unsigned int CPlayer::GetAttackPower( )
{
	...
	if( items[7].itemnum!=0 && items[7].count > 0 && items[7].durability>0 )
    {
        ...
        if( items[7].refine>0 )
        {
            ...
            {
                weaponatk += (UINT)floor(extra_refine[refine] * 0.05 * weaponatk);
            }
        }
    }
    else
    {
        attack += (UINT)floor( Stats->Level*0.2 + (Attr->Str + Attr->Estr)*0.5 + (Attr->Dex + Attr->Edex)*0.5 );
    }
    switch( weapontype )
    {
    ...
    switch(Status->Stance)
    {
        ...
        case DRIVING: //cart
        {
            ...
            if(items[136].itemnum!=0)
                porc = GServer->PatList.Index[items[136].itemnum]->speed;
            if(items[135].itemnum!=0)
                porc += GServer->PatList.Index[items[135].itemnum]->speed;
            switch ( porc ) //TODO: encontrar el porcentaje correcto
            {
                case 210:
                    mspeed = 1100;
                break;
...
unsigned int CPlayer::GetMaxHP( )
{
    ...
    for(UINT i=0;i<30;i++)
    {
        thisskill=GServer->GetSkillByID( (pskill[i])+(pskilllvl[i]));
        if( thisskill == 0 )
            continue;
        for(UINT j=0;j<3;j++ )
        {   // Passive skill fixed by Kirk
            if( thisskill->buff[j] == A_MAX_HP || thisskill->buff[j] == A_HP || thisskill->buff[j] == MAX_HP)
            {
                if( thisskill->value2[j] > 0 )
                    hpmax += hpmax * thisskill->value2[j] / 100;
                if( thisskill->value1[j] > 0 )
                    hpmax += thisskill->value1[j];
            }
        }
    }
    for(UINT i=1;i<12;i++)//cloth stats [from tomiz]
...
unsigned int CPlayer::GetMaxMP( )
{
    ...
    for(UINT i=0;i<30;i++)
    {
        ...
        if( thisskill == 0 )
            ...
            for(UINT j=0;j<3;j++ )
            {   // Passive skill fixed by Kirk
                if(thisskill->buff[j] == A_MAX_MP || thisskill->buff[j] == A_MP || thisskill->buff[j] == MAX_MP)
                {
                    if( thisskill->value2[j] > 0 )
                        maxmp += maxmp * thisskill->value2[j] / 100;
                    if( thisskill->value1[j] > 0 )
                        maxmp += thisskill->value1[j];
                }
            }
        //}
    }
...
{
    UINT amount =0;// (UINT)ceil ( Stats->MaxHP * 0.02 );
...
unsigned int CPlayer::GetMaxWeight( )
{
    ...
    weight = (Attr->Str*6)+(Stats->Level*5);
    UINT Extra = 0;
    for(UINT i=0;i<MAX_SKILL;i++)
    {
        if( cskills[i].id == 0 || cskills[i].thisskill == 0 )
